...
static int i915_adlp_hdmi_suspend(struct hda_codec *codec)
{
	...
	if (silent_streams && spec->silent_stream_type == SILENT_STREAM_KAE) {
		/*
		 * streams should remained programmed when
		 * codec goes to runtime suspend
		 */
		codec->no_stream_clean_at_suspend = 1;

		/*
		 * the system might go to S3, in which case
		 * keep-alive must be reprogrammed upon resume
		 */
